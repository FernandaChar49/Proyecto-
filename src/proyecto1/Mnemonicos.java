...
public class Mnemonicos {
    ...
    public void insertar(){
        ...
        //OPCODE correspondiente de cada instrucción del modo inherente
    ...
    /**
     * En este método se lee el archivo que contiene la tabla hash del OPCODE de cada instruccion del modo de direccionamiento inherente.
     * @return Tabla hash con valores Instruccion, OPCODE.
     */
    public Hashtable<String,String> LeerOpcodeINH(){
        ObjectInputStream fileIn;
        try{
            fileIn = new ObjectInputStream(new FileInputStream("ListaInherente.txt"));
            Inherente=(Hashtable)fileIn.readObject();
            fileIn.close();
            return Inherente;
        }catch(Exception e){
            System.out.println("No se pudo leer el archivo ListaInherente.txt o no se pudo recuperar la lista");
            return null;
        }
    }
    
    /**
     * En este método se lee el archivo que contiene la tabla hash con la longitud en bytes del operando de cada instruccion del modo de direccionamiento inherente.
     * @return Tabla hash con valores Instruccion, longitud del operando en bytes.
     */
    public Hashtable<String,Integer> LeerBytesINH(){
        ObjectInputStream fileIn;
        try{
            fileIn = new ObjectInputStream(new FileInputStream("ListaInherente.txt"));
            BytesInherente=(Hashtable)fileIn.readObject();
            fileIn.close();
            return BytesInherente;
        }catch(Exception e){
            System.out.println("No se pudo leer el archivo ListaInherente.txt o no se pudo recuperar la lista");
            return null;
        }
